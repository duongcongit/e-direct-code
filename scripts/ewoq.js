...
const ext = async () => {
    ...
    settings.Automation.AutoSubmit.status == "OFF"
    ...
    counterWorker.onmessage = () => {
        ...
        if (startButton != null) {
            ...
            if (startButton.classList.contains("enabled")) {
                hideAutoReloadBox();
                resetAutoReloadCounter();
                if (isCancelTasksAvailableNoti == false) {
                    if (isSentTaskVailableNoti == false) {
                        sendMessToSystem();
                ...
                else if (isCancelAutoReload == false) {
